...
> **Amended after review (2026-09-16), follow-up commit:** review found the `.bg` layer painted *above* the text in the nine light templates (`headline-quote-*`, `solution-feature-*`, `rox-report-*`), which declare no stacking at all; six of them also lacked `position: relative` on `.stage`. Parity couldn't see it — with no photo the layer paints nothing. The patcher now (a) emits `.{root} > *:not(.bg) { position: relative; z-index: 1; }` for any template with no positive `z-index` (the six dark templates keep their explicit ladders — the rule would push `.geo` above their overlay), and (b) inserts `position: relative` into the root rule when absent. `render-parity.ts compare` now throws if a with-photo render equals the no-photo render, and the five `.parity/with-bg/*.png` files are inspected by eye. Templates were re-patched from the pre-patch state (`git checkout 3ec73178^ -- …`) so the follow-up diff is insertions-only. **Re-review approved** (`51b9506f`: +33/−0 on exactly the nine light templates; dark six byte-identical; all five with-photo PNGs inspected — wash under every text/logo/CTA). **Product note for Jake, not a defect:** on the three light families the header/footer/masthead bands carry their own opaque backgrounds (pre-existing), so a background photo shows only in the middle band — a banded look rather than full-bleed. Making it full-bleed means making those bands transparent/translucent, a separate design decision. **Patcher hardening (minor, later):** the `hasStacking` heuristic silently skips the content rule if a light template ever gains one unrelated positive z-index — a post-patch assertion would close that; the skip guard means already-patched templates can't be upgraded in place (re-apply from the pre-patch state, as done here); the parity assertion proves the photo paints, not that it paints underneath — that remains a human check.
...
> **Preview round 1 (2026-09-16), user feedback:** "no margins — shoved against the sidebar; doesn't stick to Momentify brand and design standards." Root causes: (1) the solution pages wrap Library/History/Framework in `maxWidth: 1200; margin: 0 auto; padding: 32px 48px 80px` but render the builder bare (the old builder was bare too; its two-column grid masked it) — the canonical builder now carries that container itself; (2) the theme anchored eyebrows to `design-tokens.json`'s Space Grotesk mono + teal, but the GTM app's living convention (page consts in `general/page.tsx` etc.) is **Inter 11px/600/.14em caps in the solution accent**, h1 at weight 300, tight card radius — the `.eyebrow`, `.mono`, `.field-label` classes and the sidebar section labels now follow the app's convention (`--gtm-accent-ink` for AA), card radius 12 → 8, builder heading 22/600 → 24/400. Lesson: **the app's existing page chrome is the design standard to match, not the marketing token file.** Second preview deployed after the fix. Security follow-up `46d7ad9f` and KV cache `cb023a48` both reviewed and approved (reviewer's own probes on both routes; middleware only guards `/gtm/*` pages, so `template-preview` now requires the cookie). **Open product call:** `TemplatesGallery.tsx` "Copy link" copies an absolute `template-preview` URL, which now 401s outside the authed shell — either accept (the endpoint was never meant to be public) or repoint the button at a `/gtm/templates` deep link.

> **Preview round 2 (2026-09-16), user feedback:** "why can't I pick the template before I hit Generate?" The spec's step 3 was a pre-Generate card; Task 7 kept the picker inside `AssetPanel` (post-Generate, old-builder parity) and the review accepted the deviation — wrongly. Fixed in `47e06f0b` + `04c5f1e3`: `TemplatePicker` extracted from `AssetPanel` (new `src/components/gtm/TemplatePicker.tsx`, shared by the step-3 card and the result's "Change template"); the builder owns `templateId` (session-persisted; carousel narrows to 1:1 and clears a non-1:1 pick), passes `initialTemplateId` + `autoFill` to `AssetPanel`, whose mount asset-check auto-fills on a **confirmed** no-asset (a failed check only opens the picker — never overwrites an existing asset); the button reads **Generate + render**; post-Generate template changes in the card re-fill via an `[initialTemplateId]` effect gated by `mountedCheckDoneRef`/`busy`/`activeTemplateId`, and `onTemplateChange` reports the result's picks back to the card. Reviewed (ref timing, double-fill, hydration, carousel filter all traced). Third preview deployed.
