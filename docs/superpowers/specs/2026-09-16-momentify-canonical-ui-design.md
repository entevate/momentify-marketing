...
- `.eyebrow` — **amended after the first preview (2026-09-16):** `font-family: var(--gtm-font-body); font-size: 11px; font-weight: 600; letter-spacing: .14em; text-transform: uppercase; color: var(--gtm-accent-ink)`. The GTM app's own page chrome (`general/page.tsx` and siblings) already defines the eyebrow as Inter caps in the solution accent, and the builder sits directly beneath that chrome; the earlier Space Grotesk/teal treatment (taken from the marketing `design-tokens.json`) clashed with it. Same reasoning moved `.mono` to Inter + tabular figures, `.field-label` to the old builder's uppercase-muted form label, the sidebar section labels back to Inter in cyan, and `--gtm-radius-card` from 12px to 8px (the app's cards are 6px; the fleet's 12–14). The builder also gained the pages' `maxWidth: 1200; margin: 0 auto; padding: 32px 48px 80px` container — the solution pages wrap Library/History/Framework in it but rendered the builder bare. **Rule going forward: the app's existing page chrome is the design standard to match; the marketing token file supplies colors, not typography or shape.**
...
Grid of live template previews (existing `template-preview` GET thumbnails) in the solution accent, rendered by the shared `TemplatePicker` component (extracted from `AssetPanel`; the result's "Change template" uses the same one). Carousel narrows the grid to the 1:1 designs. The builder owns the choice (`templateId`, persisted with the session draft) and passes it to `AssetPanel` as `initialTemplateId` + `autoFill`; on Generate the copy is produced and the graphic is filled with that template automatically (button reads **Generate + render**), with any background photo applied. Left unpicked, the result opens the picker as before. After Generate the card stays live — picking another design re-fills the graphic — and `AssetPanel` reports the filled template back (`onTemplateChange`) so the card follows a change made in the result. Auto-fill runs only on a confirmed "no asset" from `asset-check`; a failed check falls back to the picker so an existing asset is never overwritten. (Preview round 3 fix — the first build kept the picker inside the result, post-Generate.)
...
```
and `<div class="bg"></div>` as the **first child** of the root (`.stage`, or `.card` for `wide-banner-11`). Layer order in the **six dark templates** (`bold-stat-*`, `wide-banner-*`, bottom → top): root hero gradient → `::before` decor pattern (a pseudo-element always paints before real children at equal z-index) → `.bg` photo → `.geo` shapes → `::after` darkening overlay → content (z-index ≥ 1). A supplied photo covers the subtle decor polygons — the intended result — while the existing overlay keeps text legible over any photo.

The **nine light templates** (`headline-quote-*`, `solution-feature-*`, `rox-report-*`) declare no stacking at all — no pseudo-layers, no z-index — so an absolutely-positioned photo would paint *above* their text (caught in review; invisible without a photo, which is why parity alone can't detect it). For any template with no positive `z-index`, the patcher also emits `.stage > *:not(.bg) { position: relative; z-index: 1; }` — lifting every non-photo child above the photo, a no-op when no photo is set because those siblings don't overlap — and guarantees `position: relative` on the root so `inset: 0` resolves against the card, not the viewport. The dark templates must NOT get that rule (it would push `.geo` above the darkening overlay). The parity script asserts the with-photo render differs from the no-photo render, and the with-photo PNGs are inspected by eye: the wash must sit under all text, logos, and CTAs. **With nothing set (`none` / `1`) the output is byte-for-byte what it is today.**
...
- `template-preview` (GET): picker thumbnails still show the designed state, like the fleet. Two hardening changes landed after review (`46d7ad9f`): it now requires `requireGtmAuth()` — `middleware.ts` only guards `/gtm/*` pages, not `/api/gtm/*`, so the route had no auth of its own (its only callers, the AssetPanel thumbnails and the templates gallery, are same-origin inside the authenticated shell and keep working) — and its reflected `data` param is escaped because escaping moved into `renderTemplate`.
- **Escaping lives in `renderTemplate`** (final design, superseding the override-only escaping above): every caller slot value is HTML-escaped at substitution; reserved palette/media keys stay raw CSS. This covers the Claude path, both overrides, `template-preview`, and the parity script in one place, so the "Claude path byte-identical" guarantee now holds for copy without markup only — `<i>Tom & Jerry</i>` stores as `<i>Tom &amp; Jerry</i>` (same glyphs). Responses still return raw values, so the slot editor never double-escapes.
- **Restored assets** (`cb023a48`): both fill routes also persist the raw filtered `slots` (object) / `cards` (6-element array) at `<assetKvKey>:slots`; `asset-check` returns them; `AssetPanel` seeds its editor and media re-render from that on mount, so a graphic filled in a previous session gets the same controls (previously the opacity slider was a silent no-op after reload).
